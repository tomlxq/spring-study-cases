...
#### 安装 解压
 ...
 解压缩（将源文件压缩包解压至/usr/local目录下）

`tar -xf nexus-${version}-unix.tar.gz -C /opt/nexus`
...
ln -s /usr/local/nexus-3.2.1-01/bin/nexus /etc/init.d/nexus3
...
访问路径：`http://{your_private_ip}:{port}/`
